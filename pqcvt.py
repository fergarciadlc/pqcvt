...
WRITE_FUNCTION_BY_FORMAT = {
    ...
    "parquet":  lambda df, x: df.to_parquet(x, index=False, 
                                            engine="pyarrow", 
...
def cli(inputfile, format_, output, force_str):
    if not inputfile.endswith(tuple(FILE_FORMATS)):
        raise click.ClickException(f"Input file format not supported, use: {FILE_FORMATS}")
    ...
    if output:
        ...
        if format_ == "excel": output = output.replace(".excel", ".xlsx")
        WRITE_FUNCTION_BY_FORMAT[format_](df, output)
